...
for i in range(50, 200):
    num = str(i)
    label = 'Surprise'
    with open(dataset_path+'picked_dataset/test_keypoints/{}/{}_keypoints.json'.format(label, num.zfill(4)), 'r') as f:
        json_data = json.load(f)
    keypoints = json_data['people'][0]['pose_keypoints_2d']
    print(keypoints)
    color = (255,255,255)
    red_color = (0, 0, 255)
    image = face_recognition.load_image_file(dataset_path + 'picked_dataset/test/{}/{}.png'.format(label, num.zfill(4)))
    image = image[:,:,::-1].copy()
    keypoint_cor = []
